	...
	: formula(emptyString)
{
	...
	MW = accumulate(elements.cbegin(), elements.cend(), 0., [](double a, const pair<Element, int>& p)
	{
		auto& e = p.first;
		int n = p.second;
...
void Species::read(istream& is)
{
	...
	Tc         = parseNumber<double>(words[4]);
	Pc         = parseNumber<double>(words[5]);
...
ostream& operator<<(ostream& os, const Species& species)
{
	...
	os << "Species " << species.name << ", with formula: " << species.formula << " and with critical temperature = " << species.Tc << " [K]" << endl;
